...
def process_audio_chunk(chunk, pitch):
    if pitch == 1.0:
        return chunk  # No change if pitch is 1.0
    
    chunk_length = len(chunk)
    resampled_length = int(chunk_length / pitch)  # Proper pitch shifting
...
while cap.isOpened():
    ...
    if left_hand and right_hand:
        ...
        with lock:
            # Update controls based on the specified scheme:
            # - Left line length controls pitch
            # - Right line length controls volume
            # - Distance between lines controls speed
            pitch_factor = np.clip(left_line_length / max_line_length, 0.1, 2.0)
            volume = np.clip(right_line_length / max_line_length, 0.2, 2.0)
            playback_speed = np.clip(lines_distance / max_distance, 0.1, 2.0)
